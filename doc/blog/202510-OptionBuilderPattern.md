...
## Heavy OOP/Abstraction Addiction

> 你们各位前面说设计模式跟 java 无关不该喷 java ，现在又说 java 在业务开发方面 pattern aop 之类的带来很多理念和进步，但这些本质上又不是
> java 带来的，因为从几十年前到现在、各种 c++的软件系统、也很多设计模式的项目，不只是 UML 做对象系统的设计，而是很多 class
> 、功能实现中都有很多设计模式。
> 除了设计模式，另外一类是属于算法、实现方案的设计之类的，比如常见的限流、熔断算法，也是与语言无关的。
> 所以这些并不真正是由 java 带来的。
> 大家反 java ，主要是反对它的臃肿、反对 javaer 把 java 的臃肿扩散到其他语言中。

- **过度抽象**： 即使是简单的 CRUD 操作，也要引入大量的接口、抽象类、AOP、Manager、Service、DAO 等层次，导致代码结构复杂、追溯困难。
- **不必要的模式**： 在不需要 Builder 模式的地方使用 Builder，在不需要策略模式的地方使用策略，将设计模式视为“任务”而非“解决方案”。

应该说现代语言（如 Go/Rust/Kotlin/TypeScript）倾向于实用主义，更强调效率、简洁和资源消耗，是对 Java 时代那种“为了扩展而过度抽象”的有力矫正