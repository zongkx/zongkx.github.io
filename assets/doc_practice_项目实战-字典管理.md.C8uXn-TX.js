import{_ as a,c as n,o as s,a4 as p}from"./chunks/framework.IXvbZIHp.js";const q=JSON.parse('{"title":"","description":"","frontmatter":{},"headers":[],"relativePath":"doc/practice/项目实战-字典管理.md","filePath":"doc/practice/项目实战-字典管理.md"}'),e={name:"doc/practice/项目实战-字典管理.md"},i=p(`<h2 id="技术选型" tabindex="-1">技术选型 <a class="header-anchor" href="#技术选型" aria-label="Permalink to &quot;技术选型&quot;">​</a></h2><p>SpringBoot2.2.6+MySQL5.6+Vue2.0+ElementUI</p><h2 id="数据库设计" tabindex="-1">数据库设计 <a class="header-anchor" href="#数据库设计" aria-label="Permalink to &quot;数据库设计&quot;">​</a></h2><p>考虑到不同的数据字典数据结构区别很小，除了在前台页面展示的列名和列的数量不一样以外，基本上都可以沿用同一种数据表结构，所以考虑使用两张表T_DICTIONARY和T_DICTIONARY_DETAIL，前者用来维护所有的数据字典表的表信息，包括表名、列名、标识、是否在用等，后者用来保存具体的某一个数据字典的数据，这里通过t_did关联前者的主键，用于区分该表的数据具体属于哪一张。后者的通用字段有五个，分别是col1到col5，根据需要从前到后存储数据字段的数据，而列名的显示通过T_DICTIONARY的字段t_col_name来保存,需要用户在录入数据的时候输入列名称,数据库存储的时候.用逗号隔开。</p><h2 id="系统设计" tabindex="-1">系统设计 <a class="header-anchor" href="#系统设计" aria-label="Permalink to &quot;系统设计&quot;">​</a></h2><p>系统采用前后端分离的模式，后台统一返回封装数据，前台对数据进行渲染，前台通过路由来进行数据表格页面的切换。</p><h2 id="代码实现" tabindex="-1">代码实现 <a class="header-anchor" href="#代码实现" aria-label="Permalink to &quot;代码实现&quot;">​</a></h2><h3 id="后台" tabindex="-1">后台 <a class="header-anchor" href="#后台" aria-label="Permalink to &quot;后台&quot;">​</a></h3><p>核心接口包括主表t_dictionary:</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>    @PostMapping(&quot;/list&quot;)</span></span>
...
<span class="line"><span>    }</span></span></code></pre></div><p>以及其详情表的接口</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>@GetMapping(&quot;/query/{did}&quot;)</span></span>
...
<span class="line"><span>    }</span></span></code></pre></div><p>后台代码较少,主要提供两表的CRUD功能即可.</p><h3 id="前台" tabindex="-1">前台 <a class="header-anchor" href="#前台" aria-label="Permalink to &quot;前台&quot;">​</a></h3><p>前台主要包括两部分:左侧列表和右侧表格内容.左侧列表第一个菜单默认为对主表的管理[综合管理],点击后右侧显示主表的表格数据,添加数据后左侧列表中多出一个菜单,点击后即可对该[新增的数据字典表]进行数据维护.</p><p>Dictionary.vue 路由配置:</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span> routes: [</span></span>
<span class="line"><span>    {</span></span>
...
<span class="line"><span>                &lt;/el-menu-item&gt;</span></span>
<span class="line"><span>              &lt;/el-menu-item-group&gt;</span></span></code></pre></div><p>All.vue 除了对t_dictionary的增上改查以外,在添加操作之后要对其父路由:Dictionary进行数据重新加载的操作. 在添加的回调里面,完成对Dictionary.vue的refresh方法调用</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>                this.$emit(&#39;refresh&#39;, &quot;&quot;);</span></span></code></pre></div><p>另外由于设计的字段是通用字段,所以在添加数据的时候需要同时添加字典表的字段名称,用于前台显示.</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span>          dictionaryColName:[{ required: true, message: &#39;请输入列名称(逗号隔开,最多支持五列)&#39;, trigger: &#39;blur&#39; }],</span></span></code></pre></div><p>Detail.vue 需要动态加载列和列名称</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code"><code><span class="line"><span> &lt;el-table-column</span></span>
...
<span class="line"><span>          this.$message.error(&quot;网络异常&quot;);</span></span>
<span class="line"><span>        })</span></span>
<span class="line"><span>      },</span></span></code></pre></div><p>同样的在添加页面修改页面都需要动态显示列和列名称.</p><hr>`,25),l=[i];function t(c,o,r,d,u,h){return s(),n("div",null,l)}const y=a(e,[["render",t]]);export{q as __pageData,y as default};
